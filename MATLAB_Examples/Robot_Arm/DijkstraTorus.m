...
while true
    ...
    if (i>1 && i<=nrows)
        i_north = i-1;
        j_north = j;
        update (i_north,j_north,min_dist+1,current);
    end
    if i==1
        i_north = nrows;
        j_north = j;
    ...
    if (i>1 && i<nrows)
        i_south = i+1;
        j_south = j;
        update (i_south,j_south,min_dist+1,current);
    end
    if i==nrows
        i_south = 1;
        j_south = j;
    ...
    if (j>1 && j<=ncols)
        j_west = j-1;
        i_west = i;
        update (i_west,j_west,min_dist+1,current);
    end
    if j==1
        i_west = i;
        j_west = ncols;
    ...
    if (j>1 && j<ncols)
        j_east =j+1;
        i_east = i;
        update (i_east,j_east,min_dist+1,current);
    end
    if j==ncols
        i_east = i;
        j_east = 1;
